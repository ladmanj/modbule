...
#define crc16(a,b)		(__bswap16(HAL_CRC_Calculate(&hcrc, (uint32_t*)(a), (b))))
#endif
...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
    	...
    	uint32_t timeout = (req_length * 11000)/ctx_rtu->baud; //number of bytes * 10 bits + 10% / bits per second * 1000 milliseconds
    	HAL_UART_Transmit((UART_HandleTypeDef *)ctx_rtu->device, req, req_length, timeout);
    	return req_length;
#endif
#if HAVE_DECL_TIOCM_RTS
    }
...
#else
//	modbus_rtu_t *ctx_rtu = ctx->backend_data;
	//HAL_UART_Receive((UART_HandleTypeDef *)ctx_rtu->device, rsp, rsp_length, 100);

	for(int i=0;i<rsp_length;i++)
	{
		if(nr_received_bytes())
			rsp[i] = read_received();
	}
	return rsp_length;
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    if (ctx->debug) {
        printf("Opening %s at %d bauds (%c, %d, %d)\n",
               ctx_rtu->device, ctx_rtu->baud, ctx_rtu->parity, // fixme
...
//    modbus_rtu_t *ctx_rtu = ctx->backend_data;
    ...
    int64_t timeout = (uint32_t)(tv->tv_sec*1000+tv->tv_usec/1000);
    timeout += HAL_GetTick();
    do{
    	__WFI();
    	HAL_WWDG_Refresh(&hwwdg);
    	if(nr_received_bytes()>=length_to_read)
    	{
    		s_rc = 1;
    	}
    	else
    ...
    }while((s_rc == 0) && ((timeout - HAL_GetTick()) > 0));
#endif
...
#else
modbus_t* modbus_new_rtu(UART_HandleTypeDef *device)
{
    modbus_t *ctx;
    modbus_rtu_t *ctx_rtu;
