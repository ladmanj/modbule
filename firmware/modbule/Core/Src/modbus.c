...
#else
const char *modbus_strerror(int errnum) {return "!";}
#endif
...
#ifndef MODBULE_MCU
        const char * const name = MSG(is_input ? "read_input_registers" : "read_registers");
#endif
        int nb = (req[offset + 3] << 8) + req[offset + 4];
        /* The mapping can be shifted to reduce memory consumption and it
           doesn't always start at address zero. */
